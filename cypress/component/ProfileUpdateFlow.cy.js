...
describe('Profile Update and Logout Flow', () => {
  ...
  it('completes the profile update and logout flow', () => {
    ...
    const mockAuthContext = {
      ...
      updateUser: cy.stub().as('updateUserStub').callsFake((updatedUser) => {
        // Update the user object in the mock context
        mockAuthContext.user = { ...mockAuthContext.user, ...updatedUser };
      }),
    };
    ...
    // Step 7: Verify that updateUser was called with the updated data
    cy.get('@updateUserStub').should('have.been.calledOnceWith', {
      id: 'user123',
    ...
    // Step 8: Verify redirection back to the Main Page
    cy.contains('Welcome to Benchat, UpdatedFirstName!').should('be.visible');

    // Step 9: Log out using the custom Logout button
    cy.get('.buttons').within(() => {
      cy.get('button').contains('Logout').scrollIntoView().click(); // Scroll into view before clicking
    });

    // Step 10: Verify redirection to the Login page
    cy.contains('Login').should('be.visible');

    // Step 11: Confirm all steps have completed successfully
